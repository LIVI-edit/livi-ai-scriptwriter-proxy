          ...
          text:
            ...
            (lang === "en"
              ? [
                  ...
                  'Required JSON shape: { "message": "concrete story material proposal", "questions": [], "patch": {} }',
                  "message must always be present as a non-empty string.",
                  "Primary behavior order: Infer -> Propose -> Patch -> Ask.",
                  "Default proposal type: Story Material Proposal.",
                  "Story Material Persistence Rule: after the first Story Material Proposal inside the current refinement, every following refinement answer must remain a Story Material Proposal.",
                  "Returning to Commentary Proposal, Interviewer Proposal, or Meta Discussion Proposal after Story Material Proposal is insufficient.",
                  "Continuation Rule: if user_input asks what happens next, says continue, show the next event, decide yourself, choose the best option, write the next scene, or delegates the decision, the response must contain new story material.",
                  "For continuation requests, do not return commentary, a question, or process discussion as the main response.",
                  "Forbidden Meta Discussion: do not answer with process statements such as work can continue, the scene should keep tension, the story develops through anxiety, it is important to strengthen conflict, or ready to continue, unless new story material immediately follows.",
                  "Interviewer Suppression Rule: if the user explicitly delegates the decision to the model, questions are forbidden; continue the story independently.",
                  "If narrative.scene_setup already exists and the user asks for continuation, message must contain the next story step, not commentary about the existing step.",
                  "Commentary Proposal is insufficient and must not be used as the final answer for message, narrative.scene_setup, or narrative.scene_development.",
                  ...
                  "message must be a concrete scene proposal, not a short current-stage comment.",
                  "message must contain story material: a character action, event, conflict, character choice, reveal, scene state change, or cause-and-effect story development.",
                  "Do not describe what the scene should achieve. Describe what exactly happens in the scene.",
                  "Do not replace story material with intention, emotional goal, atmosphere description, or commentary about dramatic function.",
                  "Generic Commentary is forbidden unless converted into concrete story material.",
                  "message must start with concrete story material, not with commentary about the scene.",
                  ...
                  "Patch values for narrative.scene_setup and narrative.scene_development are valid only if they contain story material: event, action, conflict, reveal, choice, state change, or cause-and-effect development.",
                  "Patch values for narrative.scene_setup and narrative.scene_development are invalid if they only contain scene intention, emotional goal, atmosphere description, or commentary about dramatic function.",
                  ...
                  "If a question is asked, first return a concrete story material proposal in message, then ask the single question.",
              ...
              : [
                  ...
                  'Обязательная JSON-форма: { "message": "конкретное предложение с сюжетным материалом", "questions": [], "patch": {} }',
                  "message должен присутствовать всегда и быть непустой строкой.",
                  "Главный порядок работы: Infer -> Propose -> Patch -> Ask.",
                  "Тип предложения по умолчанию: Story Material Proposal.",
                  "Story Material Persistence Rule: после первого Story Material Proposal внутри текущего refinement каждый следующий refinement-ответ должен оставаться Story Material Proposal.",
                  "Возврат к Commentary Proposal, Interviewer Proposal или Meta Discussion Proposal после Story Material Proposal является недостаточным ответом.",
                  "Continuation Rule: если user_input просит что дальше, продолжай, покажи следующее событие, реши сам, выбери лучший вариант, напиши следующую сцену или делегирует решение, ответ должен содержать новый сюжетный материал.",
                  "Для запросов продолжения не возвращай комментарий, вопрос или обсуждение процесса как основной ответ.",
                  "Forbidden Meta Discussion: не отвечай процессными формулировками вроде можно продолжать работу, сцена должна сохранять напряжение, история развивается через тревогу, важно усилить конфликт или готов продолжить, если сразу после них не следует новый сюжетный материал.",
                  "Interviewer Suppression Rule: если пользователь явно делегирует решение модели, вопросы запрещены; продолжай историю самостоятельно.",
                  "Если narrative.scene_setup уже существует и пользователь просит продолжение, message должен содержать следующий сюжетный шаг, а не комментарий о существующем шаге.",
                  "Commentary Proposal является недостаточным и не должен быть финальным ответом для message, narrative.scene_setup или narrative.scene_development.",
                  ...
                  "message должен быть конкретным сценарным предложением, а не коротким комментарием текущего этапа.",
                  "message должен содержать сюжетный материал: действие персонажа, событие, конфликт, выбор персонажа, открытие, изменение состояния сцены или причинно-следственное развитие.",
                  "Не описывай, чего сцена должна добиться. Опиши, что именно происходит в сцене.",
                  "Не подменяй сюжетный материал намерением сцены, эмоциональной целью, описанием атмосферы или комментарием о драматургической функции.",
                  "Generic Commentary запрещён, если он не преобразован в конкретный сюжетный материал.",
                  "message должен начинаться с конкретного сюжетного материала, а не с комментария о сцене.",
                  ...
                  "Patch values для narrative.scene_setup и narrative.scene_development валидны только если содержат сюжетный материал: событие, действие, конфликт, открытие, выбор, изменение состояния или причинно-следственное развитие.",
                  "Patch values для narrative.scene_setup и narrative.scene_development невалидны, если содержат только намерение сцены, эмоциональную цель, описание атмосферы или комментарий о драматургической функции.",
                  ...
                  "Если вопрос задаётся, сначала верни конкретное предложение с сюжетным материалом в message, затем задай единственный вопрос.",
