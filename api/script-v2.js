...
- Дать содержательный development-ответ человеческим языком, а не короткую заглушку.
- Явно подтвердить, какую сцену система берёт за основу.
- Коротко объяснить, как система поняла цель видео и направление дальнейшей сборки.
- Сразу добрать через patch минимум полезного смыслового каркаса: summary / setup / development / message, если это можно вывести без конфликта с пользовательским вводом.
- Не открывать новые 3 сцены.
- Не давать фейковый финал.
- При необходимости задай максимум 1 короткий вопрос, только если без него реально не хватает одного критичного опорного смысла.
...
- message должен подтверждать сцену, цель и следующий смысловой вектор.
- Если можно не спрашивать, не спрашивай.
- Не обещай final assembly.
- Не делай Alignment на этом шаге.
- patch содержит только реально уместные уточнения.
- Без markdown.
- Без пояснений.
`.trim()
      : `
...
- Не дублировать известные данные из UI, selection и уже собранного каркаса.
...
- Добрать только реально недостающий смысловой каркас для minimum usable readiness.
- Задать максимум 1 короткий уточняющий вопрос, только если без него действительно не хватает одного критичного блока.
- Если minimum usable каркас уже достаточен, не жди пустых подтверждений и сразу выдай Alignment как явный предфинальный слой перед Build.
...
- message обязателен.
- Если ready_hint=false, response_stage должен быть "refinement", а message должен быть содержательным refinement-ответом, а не короткой заглушкой.
- Если нужно спросить, задай только один вопрос и только по реально критичному недостающему смыслу.
- Если ready_hint=true, response_stage должен быть "alignment", а message должен быть именно Alignment и явно содержать: о чём видео, для чего видео, какая сцена взята за основу, какой каркас уже собран и что будет собрано после Build.
...
function buildRefinementInput({ blueprint, userMessage, language }) {
  ...
  const langRu = language !== "en";
  const systemState = blueprint?.system_state || {};
